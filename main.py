...
def do_sch(sch: tir.Schedule):
    ...
    bx, vx, tx, xi = sch.split(x, factors=[None, V_Thread_X, Block_Size_X, INNER_Size_X])
    by, vy, ty, yi = sch.split(y, factors=[None, V_Thread_Y, Block_Size_Y, INNER_Size_Y])
...
def profile(mod):
    ...
    num_runs = 10
    timer_cuda_mod = cuda_mod.time_evaluator(cuda_mod.entry_name, device, number=num_runs)
    time = timer_cuda_mod(cuda_a, cuda_b, cuda_c).mean
    gflops = 2 * M * N * K / (time * 1e3) / 1e6
    print("average time cost of %d runs = %g ms, %g GFLOPS." % (num_runs, time * 1e3, gflops))
